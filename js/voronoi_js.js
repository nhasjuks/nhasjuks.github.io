...
var Voronoi = {
	...
	draw: function() {
		...
		//this.drawSites(ctx); //draws points!
...
/* New */
var type=1, isDots=false;
function BtnGenerate(){
	Voronoi.clearSites();
	BtnAdd();
}
function BtnAdd(){
	//Voronoi.generateSites(parseInt(document.getElementById('voronoiNumberSites').value,10));
	var num=document.getElementById('voronoiNumberSites').value;
	if (num<=0) num=1;
	else if (num>100000) num=100000;
	Voronoi.generateSites(num);
	BtnDraw(-1);
}
function BtnDraw(a){
	switch(a){
		case 0: type=0; break;
		case 1: type=1; break;
		case 2: isDots=false; break;
		case 3: isDots=true; break;
	}
	if (type==0) Voronoi.drawCells();
	else if (type==1) Voronoi.draw();
	
	if (isDots) Voronoi.drawOnlySites();
}
//document.getElementById('voronoiCanvas').addEventListener('click', function() { BtnDraw(-1); }, false);
